        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {categories.map((category, index) => (
            <motion.div
              ...
              className={`relative ${category.bgColor} rounded-2xl overflow-hidden cursor-pointer group hover:scale-105 transition-transform duration-300 min-h-[200px] shadow-lg hover:shadow-xl`}
            >
              <div className="flex h-full p-6">
                {/* Text Content */}
                <div className="flex-1 flex flex-col justify-center">
                  <h3 className={`text-xl font-bold ${category.textColor} leading-tight mb-2`}>
                    {category.title}
                  </h3>
                </div>
